...
This project studies a formal extension of the Cube Rule of Food, a morphology-based framework originating with Phosphatide's Cube Rule post~\cite{phosphatide_gutfi} and later elaborated by André Arko at \url{https://cuberule.com/}~\cite{arko_cube_rule_food}, using a rank-3 tensor $T$ over three categorical axes.
...
The Cube Rule of Food is a morphology-based classification scheme that assigns a dish according to where structural starch appears on the six faces of an idealized cube. The core idea originates with Phosphatide's Cube Rule post~\cite{phosphatide_gutfi}, while the formulation used here follows André Arko's later presentation, which retrospectively casts the framework as a grand unified theory of food identification and extends it with the salad, cake, and nachos classes~\cite{arko_cube_rule_food}.
...
Figure~\ref{fig:cube-rule-types} adapts the Cube Rule morphology diagram in schematic form from Arko's presentation~\cite{arko_cube_rule_food}, itself an extension and popularization of Phosphatide's original Cube Rule post~\cite{phosphatide_gutfi}.

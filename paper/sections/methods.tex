...
\input{sections/tensor_axis_figure}
...
Accordingly, salad cells are generated and analyzed only at \texttt{starch\_type=none}, while non-salad morphologies require a non-\texttt{none} starch type.
Figure~\ref{fig:cube-rule-types} summarizes the nine cube-type morphologies used on axis $i$.
...
This prompt can be viewed as a small \term{prompt taxonomy}: it separates structural morphology instructions, axis-definition instructions, and admissibility constraints so that each part of the classification problem is stated explicitly.
...
The API call uses \term{schema-constrained generation} rather than free-form text parsing: candidates are returned through a tool payload with typed fields (\texttt{shortname}, \texttt{description}, \texttt{is\_real}, \texttt{confidence}, and \texttt{rationale\_brief}).
Operationally, this is also a practical form of \term{grammar-constrained decoding}, since only outputs that satisfy the required field structure can be returned.
...
The inclusion of canonical Cube Rule examples functions as lightweight \term{retrieval-augmented generation}: the model is not asked to classify from an unconstrained prior, but from an in-prompt bank of morphology-defining exemplars.
...
In that sense, the pipeline combines stochastic proposal with \term{guided decoding}: the model is steered both at generation time and at acceptance time toward outputs that satisfy the ontology.
...
At a descriptive level, this reflects a weak \term{manifold hypothesis}: foods that are nearby in ontology space tend to exhibit related occupancy patterns rather than arbitrary dispersion across the tensor.

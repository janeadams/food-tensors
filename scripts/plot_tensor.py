...
def render_stage_3d(stage_df: pd.DataFrame, title_prefix: str, slug: str, export_png: bool) -> None:
    ...
    fig.update_layout(
        ...
        scene=dict(
            ...
            yaxis_title="Protein Type",
            zaxis_title="Starch Type",
            xaxis=dict(
                categoryorder="array",
                categoryarray=cube_order_labels,
                tickfont=dict(size=14),
                title=dict(text="<br><br>Cube Type", font=dict(size=19)),
            ),
            yaxis=dict(
                categoryorder="array",
                categoryarray=protein_order_labels,
                tickfont=dict(size=14),
                title=dict(text="<br><br>Protein Type", font=dict(size=19)),
            ),
            zaxis=dict(
                categoryorder="array",
                categoryarray=starch_order_labels,
                tickfont=dict(size=14),
                title=dict(text="<br><br>Starch Type", font=dict(size=19)),
            ),
        ),

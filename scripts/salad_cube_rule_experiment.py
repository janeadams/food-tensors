...
def write_outputs(rows: list[dict[str, Any]], model: str) -> None:
    ...
    violating_rows = [row for row in rows if not bool(row.get("meets_cube_rule_salad", False))]
    payload = {"model": model, "items": violating_rows}
    ...
    tex_lines = [
        ...
        "\\centering",
        "\\small",
        "\\begin{tabular}{p{0.18\\textwidth}p{0.08\\textwidth}p{0.10\\textwidth}p{0.14\\textwidth}>{\\raggedright\\arraybackslash}p{0.42\\textwidth}}",
        "\\toprule",
        "Food & Salad? & Cube Type & Starch Faces & Rationale \\\\",
        "\\midrule",
    ]
    if violating_rows:
        for row in violating_rows:
            ...
            faces = row.get("starch_faces_present", [])
            face_text = ", ".join(faces) if faces else "none"
            face_text = latex_escape(face_text)
            rationale = latex_escape(str(row.get("rationale_brief", "")))
            tex_lines.append(f"{food} & {meets} & {cube_type} & {face_text} & {rationale} \\\\")
    else:
        tex_lines.append("\\multicolumn{5}{l}{No violations among tested salad-named foods.} \\\\")
    tex_lines.extend(
        [
            "\\bottomrule",
            "\\end{tabular}",
            "\\caption{Mini-experiment: only foods containing the word salad were tested, and only violations of the Cube Rule salad definition (no structural starch faces) are reported.}",
